...
export default function TournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // הגדרת פונקציה ליצירת שלב נוק-אאוט
  const handleGenerateKnockout = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/tournaments/${id}/generate-knockout`, 
        {
          method: 'POST',
          headers: {
            ...
            <>
              ...
              {tournament.format.includes('group') && (
                <Button
                  ...
                  disabled={!isAllGroupMatchesCompleted}
                >
      ...
      <div className="mt-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <TabsContent value="players" className="space-y-4">
            ...
            {convertedPlayers && convertedPlayers.length > 0 ? (
              <PlayersList players={convertedPlayers} />
            ) : (
              <div className="text-center text-gray-500 py-6">אין שחקנים להצגה</div>
            )}
          </TabsContent>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-blue-200">
                      <th className="py-2 text-right">מקום</th>
                      <th className="py-2 text-right">שחקן</th>
                      <th className="py-2 text-center">נצחונות</th>
                      <th className="py-2 text-center">הפסדים</th>
                      <th className="py-2 text-center">יחס נקודות</th>
                    </tr>
                  </thead>
                  <tbody>
                    {tournament.players
                      ...
                      .map((stats, index) => (
                        <tr key={stats.player.id} className={index % 2 === 0 ? "bg-white" : "bg-blue-50"}>
                          <td className="py-2 px-4 text-right">
                            {index === 0 && (
                              <div className="inline-flex items-center justify-center w-6 h-6 bg-yellow-400 text-yellow-900 rounded-full">
                                1
                              </div>
                            )}
                            {index === 1 && (
                              <div className="inline-flex items-center justify-center w-6 h-6 bg-gray-300 text-gray-800 rounded-full">
                                2
                              </div>
                            )}
                            {index === 2 && (
                              <div className="inline-flex items-center justify-center w-6 h-6 bg-amber-600 text-amber-100 rounded-full">
                                3
                              </div>
                            )}
                            {index > 2 && (
                          ...
                          <td className="py-2 px-4 text-center text-green-600">{stats.wins}</td>
                          <td className="py-2 px-4 text-center text-red-600">{stats.losses}</td>
                          <td className="py-2 px-4 text-center">
                            <span className={stats.pointsDiff > 0 ? "text-green-600" : "text-red-600"}>
